...
import hashlib
import json
from models.cnn_joint_classifier_cifar import ClassifierAutoencoder
...
def train_classifier_autoencoder(model, train_loader, val_loader, device, epochs=20, lr=1e-3,
...
if __name__ == "__main__":
    ...
    run_id = f"cnn_joint_classifier_{args.latent_dim}_f{args.initial_features}_{config_hash}"
    ...
    final_model_path = f'{run_dir}/cnn_joint_classifier_full_model_{run_id}.pth'
    ...
    if resume_from_checkpoint:
        print(f"Resuming CNN joint classifier training from checkpoint: {classifier_checkpoint}")
    # Save the trained model after self-supervised phase
    torch.save(model.state_dict(), f'{run_dir}/cnn_self_supervised_model.pth')

    model, clf_history = train_classifier_autoencoder(
