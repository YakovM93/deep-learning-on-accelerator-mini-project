...
import hashlib
import json
from models.resnet_autoencoder import ResNetEncoderClassifier
...
if __name__ == "__main__":
    ...
    run_id = f"resnet_joint_classifier_{args.latent_dim}_{config_hash}"
    ...
    final_model_path = f'{run_dir}/resnet_joint_classifier_full_model_{run_id}.pth'
    ...
    # Start Training Joint Classifier
    print("\nTraining Joint Classifier")
    print("-" * 50)

    # If there's a saved checkpoint and we want to resume, load it
    resume_from_checkpoint = args.resume and os.path.exists(classifier_checkpoint)
    if resume_from_checkpoint:
        print(f"Resuming ResNet joint classifier training from checkpoint: {classifier_checkpoint}")
    # Save the trained model after self-supervised phase
    torch.save(model.state_dict(), f'{run_dir}/ResNet_joint_encoder_classifier_model.pth')

    model, clf_history = train_joint_encoder_classifier(
        model, train_loader, val_loader,
        args.device, epochs=args.epochs, lr=args.lr,
        checkpoint_dir=args.checkpoint_dir, resume=resume_from_checkpoint,
    ...
    print(f"\nSummary:")
    print(f"{'='*50}")
    print(f"Model: ResNet-based Encoder and Classifier")
