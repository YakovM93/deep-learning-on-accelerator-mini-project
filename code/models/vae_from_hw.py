...
class EncoderCNN(nn.Module):
    def __init__(self, in_channels, out_channels):
        ...
        for i in range(len(channels) -1):
            modules.extend([
                nn.Conv2d(channels[i] , channels[i+1] , 5 , 2 , 2) ,
...
class VAE(nn.Module):
    def __init__(self, features_encoder, features_decoder, in_size, z_dim):
        ...
        self.calc_variance        =  nn.Linear(n_features,  z_dim)   
        self.calc_reconstruction  =  nn.Linear(z_dim     ,  n_features)
        # ========================
    ...
    def encode(self, x):
        ...
        # ====== YOUR CODE: ======
        # Feature extraction
        feature_map = self.features_encoder(x)
        # flatten batch - wise
        flattened_features = feature_map.reshape(feature_map.shape[0] , -1) 
        # log variance and latent distribution
        mu = self.calc_mean(flattened_features)
        log_sigma2 = self.calc_variance(flattened_features)

        # re-parametrization trick :  differentiable sample from latent space
        std_dev = torch.exp(0.5 * log_sigma2) 
    ...
    def decode(self, z):
        ...
        # ====== YOUR CODE: ======
        out = self.calc_reconstruction(z)\
                .view(-1 , *self.features_shape)
        x_rec = self.features_decoder(out)
        # ========================
...
def vae_loss(x, xr, z_mu, z_log_sigma2, x_sigma2):
    ...
    # ====== YOUR CODE: ======
    z_dim = z_mu.shape[1] # dx
    N = x.shape[0]

    data_loss = torch.sum((x-xr).pow(2))  / (x_sigma2 * x.numel())
   
    cov_matrix = z_log_sigma2.exp()
    tr_cov= torch.sum( cov_matrix, dim = 1)
    mean_norm = torch.sum(z_mu.pow(2) , dim =1 )

    log_det_cov = torch.sum(z_log_sigma2 ,dim = 1)
    kldiv_loss = torch.sum(tr_cov + mean_norm  - z_dim - log_det_cov ) / N
